xdescribe('Tests that will pass', () => {
  it('should pass if exact match', () => {
    ...
    expect(string).toBe('Paco');
  });
  it('should pass if recursively match (object)', () => {
    const obj = {name: 'Paco', age: 27};
    expect(obj).toEqual({name: 'Paco', age: 27});
  });
...
xdescribe('Tests that will not pass', () => {
  it('should fail if not exact match (object)', () => {
    const obj = {name: 'Paco', age: 27};
    expect(obj).toBe({name: 'Paco', age: 27});
  });
...
xdescribe('Extra matchers', () => {
    ...
    it('toMatch', () => {
      expect('Un texto largo que tiene Paco dentro de sus palabras').toMatch('Paco');
      expect('Un texto largo que tiene Paco dentro de sus palabras').toMatch(/Paco/);
      expect('Un texto largo que tiene Paco dentro de sus palabras').toMatch(/Paco$/);
    });

    it('toContain', () => {
      const array = ['Paco', 'Jose', 'Maricarmen', 'Ana'];
      expect(array).toContain('Maricarmen');
